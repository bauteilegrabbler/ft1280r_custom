/*
  ...
  Rev 6.0 - December 24th, 2012, Andreas Czechanowski
          - Added read and write functions that allow transfer of two
            register address bytes preceding data.
...
/**
 * writes 1 byte to a slave. Usually, this is a register address byte.
 *
 * @param address          I2C bus slave address (7 bit format)
 * @param registerAddress  address pointer within slave to set
 */
...
/**
 * convenience funtion with integer arguments,
 * writes 1 byte to a slave. Usually, this is a register address byte.
 *
 * @param address          I2C bus slave address (7 bit format)
 * @param registerAddress  address pointer within slave to set
...
/**
 * writes 2 bytes to a slave. Usually, this is a register address byte
 * and one data byte.
 *
 * @param address          I2C bus slave address (7 bit format)
 * @param registerAddress  address within slave to access
 * @param tdata            data byte to write to slave
 */
uint8_t I2C::write(uint8_t address, uint8_t registerAddress, uint8_t tdata)
{
  returnStatus = 0;
  returnStatus = start(); 
  if(returnStatus){return(returnStatus);}
...
/**
 * convenience funtion with integer arguments,
 * writes 2 bytes to a slave.
 *
 * @param address          I2C bus slave address (7 bit format)
 * @param registerAddress  address within slave to access
 * @param tdata            data byte to write to slave
 */
uint8_t I2C::write(int address, int registerAddress, int tdata)
{
  return(write((uint8_t) address, (uint8_t) registerAddress, (uint8_t) tdata));
}


/**
 * writes (1 + strlen) bytes to a slave.
 *
 * @param address          I2C bus slave address (7 bit format)
 * @param registerAddress  address within slave to access
 * @param tstr             NUL-terminated string to write to slave
 */
uint8_t I2C::write(uint8_t address, uint8_t registerAddress, char *tstr)
{
  uint8_t bufferLength = strlen(tstr);
  returnStatus = 0;
  returnStatus = write(address, registerAddress, (uint8_t*)tstr, bufferLength);
  return(returnStatus);
}


/**
 * writes (1 + numberBytes) bytes to a slave. Usually, this is
 * one register address byte and n data bytes.
 *
 * @param address          I2C bus slave address (7 bit format)
 * @param registerAddress  address within slave to access
 * @param tdata            array of bytes to write to slave
 * @param numberBytes      byte count in tdata to write, arbitrary count
 */
uint8_t I2C::write(uint8_t address, uint8_t registerAddress, uint8_t *tdata, uint8_t numberBytes)
{
  returnStatus = 0;
  returnStatus = start();
  if(returnStatus){return(returnStatus);}
...
/**
 * writes (2 + numberByte) bytes to a slave. Usually, this is
 * a register address in two bytes and n data bytes.
 *
 * @param address          I2C bus slave address (7 bit format)
 * @param registerAddrHigh high byte of address within slave to access
 * @param registerAddrLow  low byte of address within slave to access
 * @param tdata            array of bytes to write to slave
 * @param numberBytes      byte count in tdata to write, arbitrary count
 */
uint8_t I2C::write(uint8_t address, uint8_t registerAddrHigh,
                   uint8_t registerAddrLow, uint8_t *tdata, uint8_t numberBytes)
{
  returnStatus = 0;
  returnStatus = start();
  if(returnStatus){return(returnStatus);}
  ...
  returnStatus = sendByte(registerAddrHigh);
  if(returnStatus)
  {
    if(returnStatus == 1){return(3);}
    return(returnStatus);
  }
  returnStatus = sendByte(registerAddrLow);
  if(returnStatus)
  {
    if(returnStatus == 1){return(3);}
    return(returnStatus);
...
/**
 * reads numberBytes bytes from slave into data[]
 *
 * @param address      I2C bus slave address (7 bit format)
 * @param numberBytes  byte count to read, 0 to I2C_BUFFER_SIZE
 */
...
/**
 * reads numberBytes bytes from slave into data[], after sending
 * one register address byte to the slave.
 *
 * @param address      I2C bus slave address (7 bit format)
 * @param registerAddress  address within slave to access
 * @param numberBytes  byte count to read, 0 to I2C_BUFFER_SIZE
...
/**
 * reads numberBytes bytes from slave into dataBuffer[], after sending
 * one register address byte to the slave.
 *
 * @param address      I2C bus slave address (7 bit format)
 * @param registerAddress  address within slave to access
 ...
 * @param registerAddress  address within slave to access
 * @param numberBytes  byte count to read, 0 to I2C_BUFFER_SIZE
 * @param dataBuffer   array for data bytes received from slave
 */
...
/**
 * reads numberBytes bytes from slave into dataBuffer[], after sending
 * two register address bytes to the slave.
 *
 * @param address          I2C bus slave address (7 bit format)
 * @param registerAddrHigh high byte of address within slave to access
 * @param registerAddrLow  low byte of address within slave to access
...
uint8_t I2C::read(uint8_t address,
                  uint8_t registerAddrHigh, uint8_t registerAddrLow,
